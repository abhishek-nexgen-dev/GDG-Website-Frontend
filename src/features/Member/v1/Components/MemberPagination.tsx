...
const MemberPagination = ({
  ...
  const startIdx = totalMembers === 0 ? 0 : (currentPage - 1) * pageSize + 1;
  const endIdx = Math.min(currentPage * pageSize, totalMembers);
  ...
  return (
    <div className="flex flex-col items-center justify-between gap-4 py-3 sm:flex-row">
      ...
      <p className="text-xs text-white/40">
        ...
        <span className="font-medium text-white/80">{totalMembers}</span> members
      </p>
      ...
      <div className="relative">
        ...
        >
          ...
          <option value={50} className="bg-[#161a1f] text-white">
            50 per page
          </option>
        </select>
        <ChevronDown
